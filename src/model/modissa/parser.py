...
class SemanticModissa(Dataset):
  ...
  def __init__(self, root,    # directory where data is
    ...
    self.sensor_img_H = self.sensor["height"]
    self.sensor_img_W = self.sensor["width"]
    ...
    sensor_name = self.dataset_config['sensor']['name']
    ...
    for seq in self.sequences:
      ...
      if self.use_residual:
        for i in range(self.n_input_scans):
          ...
          exec("residual_path_" + str(i+1) + " = os.path.join(self.root, seq, sensor_name ,folder_name)")
      ...
      """
      # load poses
      pose_file = os.path.join(self.root, seq,sensor_name, "poses.txt")
      print(f"Loading poses {pose_file}")
      poses = np.array(load_poses(pose_file))
      inv_frame0 = np.linalg.inv(poses[0])

      # load calibrations
      calib_file = os.path.join(self.root, seq, sensor_name,"calib.txt")
  ...
  def __getitem__(self, dataset_index):
    ...
    # return
    return proj_full, proj_mask, proj_labels, unproj_labels, path_seq, path_sensor ,path_name, proj_x, proj_y, proj_range, \
...
class Parser():
  ...
  def __init__(self,
    ...
    if self.split == 'train' or not self.split:
      self.train_dataset = SemanticModissa(root=self.root,
                                         ...
                                         CFG=CFG,
                                         augment=self.augment_train, # set to True to augment the data
                                         gt=self.gt)
      ...
      self.valid_dataset = SemanticModissa(root=self.root,
    ...
    if self.split == 'test' or not self.split:
      if self.test_sequences:
        self.test_dataset = SemanticModissa(root=self.root,
  ...
  def to_original(self, label):
    ...
    return SemanticModissa.map(label, self.learning_map_inv)

  def to_xentropy(self, label):
    # put label in xentropy values
    return SemanticModissa.map(label, self.learning_map)

  def to_color(self, label):
    # put label in original values
    label = SemanticModissa.map(label, self.learning_map_inv)
    # put label in color
    return SemanticModissa.map(label, self.color_map)