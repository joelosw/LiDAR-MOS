...
class Trainer():
    def __init__(self, master_config, logdir, pretrained_path=None, uncertainty=False):
        # parameters
        self.CONFIG = master_config
        self.datadir = master_config['dataset']['root_folder']
        self.log = logdir
        ...
        self.parser = parserModule.Parser(root=self.datadir,
                                          ...
                                          test_sequences=None,
                                          split='train',
                                          labels=self.CONFIG["labels"]["labels"],
        ...
        epsilon_w = self.CONFIG["train"]["epsilon_w"]
        content = torch.zeros(self.parser.get_n_classes(), dtype=torch.float)
        for cl, freq in self.CONFIG["labels"]["content"].items():
            x_cl = self.parser.to_xentropy(cl)  # map actual class to xentropy class
            content[x_cl] += freq
        self.loss_w = 1 / (content + epsilon_w)  # get weights
        for x_cl, w in enumerate(self.loss_w):  # ignore the ones necessary to ignore
            if self.CONFIG['labels']["learning_ignore"][x_cl]:
                # don't weigh
                self.loss_w[x_cl] = 0
        ...
        with torch.no_grad():
            if not self.uncertainty:
                self.model = SalsaNext(self.parser.get_n_classes(), len(self.CONFIG['residual_images']['num_last_n']))
            else:
        ...
        up_steps = int(self.CONFIG["train"]["wup_epochs"] * steps_per_epoch)
        final_decay = self.CONFIG["train"]["lr_decay"] ** (1 / steps_per_epoch)
        ...
        if self.pretrained_path is not None:
            ...
            w_dict = torch.load(pretrained_path + "/SalsaNext",
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.CONFIG["train"]["max_epochs"]):
            ...
            acc, iou, loss, update_mean,hetero_l, iou_class_2 = self.train_epoch(train_loader=self.parser.get_train_set(),
                                                           ...
                                                           report=self.CONFIG["train"]["report_batch"],
                                                           show_scans=self.CONFIG["train"]["show_scans"])
            ...
            if epoch % self.CONFIG["train"]["report_epoch"] == 0:
                ...
                acc, iou, loss, rand_img,hetero_l, class_jaccard = self.validate(val_loader=self.parser.get_valid_set(),
                                                         ...
                                                         color_fn=self.parser.to_color,
                                                         save_scans=self.CONFIG["train"]["save_scans"])
            ...
            Trainer.save_to_log(logdir=self.log,
                                ...
                                info=self.info,
                                epoch=epoch,
                                w_summary=self.CONFIG["train"]["save_summary"],
                                model=self.model_single,
                                img_summary=self.CONFIG["train"]["save_scans"],
                                imgs=rand_img)
